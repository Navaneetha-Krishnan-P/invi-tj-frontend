...
const Dashboard = () => {
  ...
  // Check on mount if welcome should be shown
  useEffect(() => {
  ...
  if (error) {
    return (
      <div className="dashboard-screen">
        <div className="screen-header">
          <h2>Dashboard</h2>
          <p className="error-message">Unable to load dashboard data. Please try refreshing the page.</p>
        </div>
  ...
  return (
    <div className="dashboard-screen">
      {showWelcomeDialog && <WelcomeDialog onClose={clearWelcome} />}
      ...
      <div className={`stats-grid${animateStats ? ' animate-up' : ''}`}>
        <div className="stat-card">
          <div className="stat-icon"><MdDashboardCustomize color='345c90'/></div>
        ...
        <div className="stat-card">
          <div className="stat-icon"><TbCirclePercentageFilled color='345c90'/></div>
          <div className="stat-content">
            <h3>Win Rate</h3>
        ...
        <div className="stat-card">
          <div className="stat-icon"><GiTrophy color='345c90'/></div>
          <div className="stat-content">
        ...
        <div className="stat-card">
          <div className="stat-icon"><FaArrowTrendDown color='345c90'/></div>
          <div className="stat-content">
            <h3>Losing Trades</h3>
      ...
      <div className="dashboard-main-section">
        ...
        <div className="module-content-container">
          ...
          <div className="market-overview-card forex">
            <div className="market-overview-header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <h4>Forex Market <span style={{ color: "#71797a", marginLeft: 6, fontSize: 12 }}>(USD)</span></h4>
            ...
            <div className="market-overview-stats">
              {forexLoading ? (
                ...
                <>
                  ...
                  <div className="overview-stat">
                    <span className="stat-label">Net P/L</span>
                    <span className={`stat-value ${forexTrades.reduce((sum, t) => sum + Number(t.profit_loss), 0) >= 0 ? 'positive' : 'negative'}`}>
          ...
          <div className="market-overview-card indian">
            <div className="market-overview-header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <h4>Indian Market <span style={{ color: "#71797a", marginLeft: 6, fontSize: 12 }}>(INR)</span></h4>
              <div className="filter-group" style={{ margin: 0 }}>
                <select 
            ...
            <div className="market-overview-stats">
              {indianLoading ? (
                ...
                <>
                  ...
                  <div className="overview-stat">
                    <span className="stat-label">Net P/L</span>
                    <span className={`stat-value ${indianTrades.reduce((sum, t) => sum + Number(t.profit_loss), 0) >= 0 ? 'positive' : 'negative'}`}>
      ...
      <div className="dashboard-main-section">
        ...
        <p>Calculate your portfolio performance by entering your initial capital and selecting the market. View returns across different time periods.</p>
        
        <div className="module-content-container">
          <div className="performance-calculator-single">
          <div className="performance-calculator">
            <h3 style={{ fontSize: '18px', marginBottom: '20px', color: '#345c90', fontWeight: 700 }}>
            ...
            {capital && Number(capital) > 0 && (
              <div className="performance-results-single">
                {perfLoading ? (
                  ...
                  const selectedTrades = performanceMarket === 'INDIAN' ? indianTrades : forexTrades;
                  const currencySymbol = performanceMarket === 'INDIAN' ? '₹' : '$';
                  const filteredTrades = filterTrades(selectedTrades, performanceFilter);
                  const totalPL = filteredTrades.reduce((sum, t) => sum + Number(t.profit_loss), 0);
                  ...
                  const profitTrades = filteredTrades.filter(t => Number(t.profit_loss) > 0);
                  const lossTrades = filteredTrades.filter(t => Number(t.profit_loss) < 0);
                  const totalProfit = profitTrades.reduce((sum, t) => sum + Number(t.profit_loss), 0);
                  const totalLoss = lossTrades.reduce((sum, t) => sum + Number(t.profit_loss), 0);
                    ...
                    <div className="performance-card-single">
                      <div className="performance-details-grid">
                        <div className="perf-stat">
                          ...
                          <span className="perf-value">{performanceMarket === 'INDIAN' ? 'Indian Market' : 'Forex Market'}</span>
                        </div>
                        <div className="perf-stat">
                          <span className="perf-label">Initial Capital:</span>
                          <span className="perf-value">{currencySymbol} {capitalValue.toLocaleString()}</span>
                        </div>
                        <div className="perf-stat">
                        ...
                        <div className="perf-stat">
                          ...
                          <span className="perf-value positive">{currencySymbol} {totalProfit.toFixed(2)}</span>
                        </div>
                        <div className="perf-stat">
                          <span className="perf-label">Total Loss:</span>
                          <span className="perf-value negative">{currencySymbol} {totalLoss.toFixed(2)}</span>
                        </div>
                        <div className="perf-stat">
                          <span className="perf-label">Net P/L:</span>
                          <span className={`perf-value ${totalPL >= 0 ? 'positive' : 'negative'}`}>
                            {currencySymbol} {totalPL.toFixed(2)}
                          </span>
                        </div>
                        <div className="perf-stat">
                          <span className="perf-label">Final Capital:</span>
                          <span className={`perf-value ${finalCapital >= capitalValue ? 'positive' : 'negative'}`}>
                            {currencySymbol} {finalCapital.toLocaleString()}
                          </span>
                        </div>
                        <div className="perf-stat highlight">
                          <span className="perf-label">Performance:</span>
                          <span className={`perf-value ${performancePercent >= 0 ? 'positive' : 'negative'}`}>
      ...
      <div className="dashboard-main-section no-margin-bottom">
        ...
        <p>Detailed visual analysis of your trading performance across different markets and time periods. Use the filters to customize your view and gain insights into your trading patterns.</p>
      </div>
      ...
      <div className="chart-section profit-chart-section">
        <div className="chart-header">
          <div>
            ...
            <p style={{ color: '#7f8c8d', fontSize: '14px', margin: '8px 0 0 0' }}>
              Combined view of profit and loss across all markets and time periods. This chart displays both winning and losing trades side by side for comprehensive performance analysis.
            </p>
          </div>
        </div>
        <StockCombinedChart />
      ...
      <div className="chart-section">
        <div className="chart-header">
          <div>
            ...
            <p style={{ color: '#7f8c8d', fontSize: '14px', margin: '8px 0 0 0' }}>
              Visual breakdown of your winning versus losing trades. This simple bar chart shows the proportion of successful trades compared to losses, helping you understand your overall win rate at a glance.
            </p>
          </div>
        </div>
